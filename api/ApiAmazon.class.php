...
class ApiAmazon
{
	...
	/**
	 * obtains data result for setting $key => $value in Amazon domain
	 *
	 * @param string    $key
	 * @param int|float $value
	 *
	...
	/**
	 * echoes value associated with $key in Amazon domain as json
	 *
	 * @param string $key
	 */
	...
	/**
	 * returns value associated with $key (or all) in Amazon domain as array
	 *
	...
	public static function getPriceJSON($itemID)
	{
		...
		if (!is_null($itemID) && !is_nan($itemID))
		{
			try
			{
				...
				$result['data'] = array(
					'setPrice' => $setPriceData['NewPrice'],
					'written'  => $setPriceData['Written']
				);
				$result['success'] = true;
			} catch (Exception $e)
			{
	...
	public static function getPrice($itemID)
	{
		...
		return array(
			'NewPrice' => $aPriceChanged['currentPrice'],
			'Written'  => $aPriceChanged['written'] == 1
		);
	}
	...
	public static function getAmazonPriceData($page = 1, $rowsPerPage = 10, $sortByColumn = 'ItemID', $sortOrder = 'ASC', $itemID = null, $itemNo = null, $itemName = null, $marking1ID = null)
	{
		...
		while ($amazonPriceData = $amazonPriceDataDBResult->fetchAssoc())
		{
			...
			if ($isWrittenTimeValid)
			{
				...
				$currentDays = intval($writtenDateToNowDifference->format('%a')) + floatval($writtenDateToNowDifference->format('%h') / 24);
			}
			...
			$data['rows'][$sku] = array(
				...
				'TimeData'        => array(
					'writtenTime' => ($isWrittenTimeValid && !empty($writtenDate)) ? $writtenDate->format('d.m.Y') : '-',
					'targetDays'  => $config['MeasuringTimeFrame'],
					'currentDays' => ($isWrittenTimeValid && !empty($currentDays)) ? number_format($currentDays, 1) : '-'
				),
				'Quantities'      => array(
...
LIMIT " . (($page - 1) * $rowsPerPage) . ",$rowsPerPage";
	...
	public static function setPrice($itemID, $newPrice)
	{
		...
		} else
		{
			...
			{
				if ($priceSetsDBResult->getNumRows() > 1)
				{
					throw new RuntimeException("Item $itemID: found " . $priceSetsDBResult->getNumRows() . " price sets, expected exactly one!");
				} else
				{
