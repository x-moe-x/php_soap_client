...
class DBUtils
{
	...
	public static function buildInsert(array $aInsert, $aUnquoteKeys = array())
	{
		...
		if (is_array($aInsert))
		{
			foreach (array_keys($aInsert) as $key)
			{
				...
				if (in_array($key, $aUnquoteKeys) || strtoupper($aInsert[$key]) == 'NULL' || strtoupper($aInsert[$key]) == 'NOW()')
				{
					$v .= $aInsert[$key] . ',';
	...
	public static function buildPreparedInsert(array $array)
	{
		...
		if ($array)
		{
			...
			{
				$f .= '`' . self::quoteSmart($key) . '`,';
				if (strtoupper($array[$key]) == 'NULL' || strtoupper($array[$key]) == 'NOW()')
				{
					$sValues .= $array[$key] . ',';
				} else
				{
	 ...
	 *
	 * @param array $aUpdate      assoc array
	 * @param array $aUnquoteKeys (optional) do not quote these keys
	 * @param bool  $return_set
	 *
	 * @return string
	 */
	...
	public static function buildWhere(array $array, $sTablePrefix = '', $bReturnWhere = true)
	{
		...
		if ($array)
		{
			...
			foreach (array_keys($array) as $key)
			{
				...
				if ($array[$key] == 'NULL' || $array[$key] == 'NOW()'
				)
				{
					$out .= $array[$key] . ' &&';
	...
	/**
	 * buildOnDuplicateKeyUpdate - returns sql string for sql on duplicate key update statement
	...
	public static function buildOnDuplicateKeyUpdate(array $array, $aUnquoteKeys = array())
	{
		...
		if ($array)
		{
			...
			{
				$out .= '`' . self::quoteSmart($key) . '`=';
				if (in_array($key, $aUnquoteKeys) || strtoupper($array[$key]) == 'NULL' || strtoupper($array[$key]) == 'NOW()')
				{
					$out .= $array[$key] . ',';
				} else
				{
