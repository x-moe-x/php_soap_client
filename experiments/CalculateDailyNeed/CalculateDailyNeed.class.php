...
class CalculateDailyNeed
{
	...
	public function execute()
	{
		...
		$articleResultA = DBQuery::getInstance()->select($this->getIntervalQuery($this->config['CalculationTimeA']['Value']));
		$articleResultAWithActivationDate = DBQuery::getInstance()->select($this->getIntervalQuery($this->config['CalculationTimeA']['Value'], true));
		...
		$articleResultB = DBQuery::getInstance()->select($this->getIntervalQuery($this->config['CalculationTimeB']['Value']));
		$articleResultBWithActivationDate = DBQuery::getInstance()->select($this->getIntervalQuery($this->config['CalculationTimeB']['Value'], true));
	...
	AVG(`quantity`) + STDDEV(`quantity`) * {$this->config['StandardDeviationFactor']['Value']} AS `range`,
...
ORDER BY
	...
	private function processArticle(array $aCurrentArticle, $sAorB, $withActivationDate = false)
	{
		...
		$daysBack = $this->config['CalculationTime' . $sAorB]['Value'];
		$isNewArticle = 0;
		...
		$adjustedQuantity = $this->getArticleAdjustedQuantity(explode(',', $aCurrentArticle['quantities']), $aCurrentArticle['quantity'], $aCurrentArticle['range'], $this->config['MinimumToleratedSpikes' . $sAorB]['Value'], $this->config['MinimumOrders' . $sAorB]['Value'], $skippedIndex);
		...
		if ($sAorB === 'A')
		{
			...
			// @formatter:off
			$this->articleData[$aCurrentArticle['SKU']] = array(
				'ItemID'              => $ItemID,
		...
		else
		{
			...
			{
				$this->getLogger()->err(__FUNCTION__ . ' : wrong syntax of $sAorB : ' . $sAorB);
				die();
			}
		}
	}
	...
	private function updateArticleOnActivationDate(&$aCurrentArticle, &$daysBack, &$isNewArticle, &$skipArticle)
	{
		...
		else
		{
			...
			{
				if ($activationTimeDifference < $daysBack && $activationTimeDifference < $this->config['CalculationTimeA']['Value'])
				{
	...
	{
		DBQuery::getInstance()->insert('INSERT INTO `CalculatedDailyNeeds`' . DBUtils2::buildMultipleInsertOnDuplikateKeyUpdate($this->articleData));
	}
}
