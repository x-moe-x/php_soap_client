...
class SoapCall_SetLinkedItems extends PlentySoapCall {
	...
	public function execute() {
		try {
			...
			$setLinkedItemsDbResult = DBQuery::getInstance()
			...
			while ($row = $setLinkedItemsDbResult->fetchAssoc()) {
				...
				$preparedRequest = new RequestContainer_SetLinkedItems($row['ItemID'], $row['RelationShip'], $row['deleteLink']);
				...
				$response = $this->getPlentySoap()
					->SetLinkedItems($preparedRequest->getRequest());
