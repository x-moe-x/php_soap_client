...
class CalculateAmazonWeightenedRunningCosts {
	...
	public function execute() {
		try {
			...
			$this->getLogger()
				->info(__FUNCTION__ . ' storing to config WarehouseRunningCostsAmount = ' . number_format($valueK, 10));
			ApiAmazon::setConfig('CommonRunningCostsAmount', number_format($averageCosts, 4));
			$this->getLogger()
				->info(__FUNCTION__ . ' storing to config CommonRunningCostsAmount = ' . number_format($averageCosts, 4));
		} catch (Exception $e) {
			$this->getLogger()
				->debug(__FUNCTION__ . ' Error: ' . $e->getMessage());
		}
	}
	...
	private function getAmazonTotalNettoAndShippingByDate() {
		...
		$amazonTotalNettoAndShippingDBResult = DBQuery::getInstance()
			->select(TotalNettoQuery::getTotalNettoAndShippingCostsQuery($this->oStartDate, new DateInterval('P' . $this->nrOfDataMonths . 'M'), ApiAmazon::AMAZON_REFERRER_ID));
	...
	private function prepareANData(&$groups) {
		$amazonPerWarehouseNettoDBResult = DBQuery::getInstance()
			->select(TotalNettoQuery::getPerWarehouseNettoQuery($this->oStartDate, new DateInterval('P' . $this->nrOfDataMonths . 'M'), ApiAmazon::AMAZON_REFERRER_ID));
		$aNData = array();
	...
	private function getRelevantMonths(&$cRRatData) {
		...
		// found none...
		throw new RuntimeException('Found no sufficient running cost data within ' . $this->nrOfDataMonths . ' months, cannot compute value K');
	}
	...
	private function getAverageGeneralCosts(&$cRRatData) {
		...
		// found none...
		throw new RuntimeException('Found no sufficient general cost data within ' . $this->nrOfDataMonths . ' months, cannot compute value L');
	}
