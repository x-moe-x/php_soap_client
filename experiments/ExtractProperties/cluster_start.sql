...
FROM (
			 ...
			 FROM `ExtractedProperties` AS p
				 JOIN `ItemsBase` AS i
					 ON i.ItemID = p.ItemID
				 LEFT JOIN `PropertySiblings` AS s
					 ON
	...
	LEFT JOIN (
							SELECT
								...
								p.Value
							FROM ExtractedProperties AS p
								JOIN PropertySiblings AS s
									ON p.RecordID = s.SiblingRecordID
							GROUP BY s.SiblingRecordID, s.SiblingType) AS siblingData
		ON original.SiblingRecordID = siblingData.SiblingRecordID and original.SiblingType = siblingData.SiblingType
GROUP BY CASE WHEN (siblingData.SiblingRecordID IS NULL) THEN
	original.Key
				 ELSE
					 siblingData.Key
				 END,
