...
class SoapCall_SearchOrders extends PlentySoapCall
{
	...
	/**
	 * @param PlentySoapResponse_SearchOrders $oPlentySoapResponse_SearchOrders
	 */
	...
	/**
	 * @param PlentySoapObject_SearchOrders $oPlentySoapObject_SearchOrders
	 */
	private function processOrder($oPlentySoapObject_SearchOrders)
	{
		...
		{
			if (isset($oPlentySoapObject_SearchOrders->OrderItems->item))
			{
	...
	/**
	 * @param PlentySoapObject_OrderHead $oOrderHead
	 */
	private function processOrderHead($oOrderHead)
	{
		...
		if ($this->caughtUTF8Error)
		{
			...
			$query = 'REPLACE INTO `FailedOrderIDRange`' . DBUtils::buildInsert(array(
					'FromOrderID' => $this->lastOrderID + 1,
					'CountOrders' => intval($oOrderHead->OrderID) - ($this->lastOrderID + 1),
					'Reason'      => 'UTF-8 error'
				));
	...
	/**
	 * @param PlentySoapObject_OrderItem $oOrderItem
	 */
	private function processOrderItem($oOrderItem)
	{
		$this->aOrderItems[] = array(
